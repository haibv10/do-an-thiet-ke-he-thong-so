module cpu_top #(
  ...
  // 1 MHz enable rather than a divided clock, so the frame FSM stays in the
  // single 27 MHz domain and adds no crossing.
  clock_enable #(.divider(27)) i2c_clock_enable (
    .clk(clk), .rst_n(rst_n_sync), .tick(i2c_tick)
  );

  i2c_mmio clock_port (
    .clk(clk), .tick(i2c_tick), .rst_n(rst_n_sync), .we(we_i2c),
    .a(mem_alu_result), .wd(mem_store_data), .rd(i2c_rd),
