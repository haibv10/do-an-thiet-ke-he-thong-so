...
module address_decoder_tb;
  ...
  task automatic expect_write(
    ...
    begin
      ...
      if (we_dmem !== dmem_en || we_gpio !== gpio_en ||
          we_uart !== uart_en || we_i2c !== i2c_en)
        $fatal(1, "%s: write to %h gave dmem=%b gpio=%b uart=%b i2c=%b",
               label, address, we_dmem, we_gpio, we_uart, we_i2c);
      we_mask = 4'b0000;
    end
  endtask
  ...
  initial begin
    ...
    expect_read(32'h7000_0000, 32'd0, "unmapped 0x7");
    ...
    expect_write(32'h6000_0000, 4'b0000, 1'b0, 1'b0, 1'b1, "store to I2C");
    ...
    $display("address_decoder_tb: PASS");
    $finish;
  end
endmodule
