...
module pipe_id_ex_tb;
  ...
  initial begin
    ...
    if ({ex_RegWrite, ex_MemtoReg, ex_MemWrite, ex_MemRead,
         ex_Branch, ex_ALUSrc, ex_Jump} !== '0) $fatal(1, "reset controls");
    @(negedge clk) rst_n = 1'b1;
    @(posedge clk); #1;
    if (ex_pc !== id_pc || ex_rd1 !== id_rd1 || ex_rd2 !== id_rd2 ||
        ex_imm !== id_imm || ex_rd_idx !== id_rd_idx ||
        ex_Jump !== id_Jump) $fatal(1, "capture");
    @(negedge clk) flush = 1'b1;
    @(posedge clk); #1;
    if ({ex_RegWrite, ex_MemtoReg, ex_MemWrite, ex_MemRead,
         ex_Branch, ex_ALUSrc, ex_Jump} !== '0) $fatal(1, "flush controls");
